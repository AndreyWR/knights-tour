...
class aestrela:
    ...
    def find_solution_for(self, heuristic = lambda graph: None):
        ...
        def traverse(path, current_vertex):
            ...
            next_vertices = sorted(yet_to_visit, key=cmp_to_key(heuristic(graph)))
    ...
    # Heurística warnsdorffs
    def warnsdorffs_heuristic(self, graph):

        #Dado um grafo, retorna uma função de comparação que prioriza nós
        #com os menores movimentos subsequentes
        def comparator(a, b):
